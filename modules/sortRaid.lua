...
function M:BuildDelta(sortMode)
    ...
    local supportRoles = {}  -- Store support players for round-robin distribution
    ...
    for i, k in ipairs(keys) do
        ...
        if player.role == A.group.ROLE.SUPPORT then
            ...
            groupAssignments[newGroup] = groupAssignments[newGroup] or {}
            tinsert(groupAssignments[newGroup], player)

            if #groupAssignments[newGroup] >= 5 then
                newGroup = newGroup + 1
            end
        end
    ...
    -- Distribute support roles in round-robin fashion
    local supportIndex = 1
    for group, groupPlayers in ipairs(groupAssignments) do
        if supportRoles[supportIndex] then
            tinsert(groupPlayers, supportRoles[supportIndex])
            supportIndex = supportIndex + 1
        end
    end

    -- Flatten groupAssignments back into the keys array
    keys = {}
    for group, groupPlayers in ipairs(groupAssignments) do
